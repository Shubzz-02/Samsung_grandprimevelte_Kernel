...
#if 1
	if (on){
		printk("set mic_bias_volt=%d\n",info->mic_bias_volt);
		regulator_set_voltage(info->mic_bias, info->mic_bias_volt, info->mic_bias_volt);
...
static void pm886_hook_work(struct pm886_hs_info *info)
{
	...
	voltage = gpadc_measure_voltage(info, VOLTAGE_AVG);  //KSND get voltage of push
	msleep(50);
...
//for voice command key
		if (of_property_read_u32(pdev->dev.of_node,
					 "marvell,voc-command-press-th",
					 &hs_info->voc_command_press_th))
