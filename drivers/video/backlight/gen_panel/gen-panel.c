...
static int gen_panel_parse_dt_backlight(struct device_node *backlight_node, struct lcd *lcd)
{
	struct gen_panel_backlight_info *ginfo;
	struct device_node *np;
	int arr[MAX_BRT_VALUE_IDX * 2];
	bool outdoor_mode_en;
	u32 panel_id;
	int i;

	lcd->bd = of_find_backlight_by_node(backlight_node);
	ginfo = (struct gen_panel_backlight_info *)bl_get_data(lcd->bd);
	 ...
	 */
	for_each_child_of_node(backlight_node, np) {
		of_property_read_u32(np, "brt-panel-id", &panel_id);
		if (panel_id == lcd->id) {
...
int gen_panel_probe(struct device_node *node, struct lcd *lcd)
{
	...
	if (IS_ENABLED(CONFIG_OF)) {
		...
		backlight_node = gen_panel_find_dt_backlight(node,
				"gen-panel-backlight");
		if (backlight_node) {
			gen_panel_parse_dt_backlight(backlight_node, lcd);
